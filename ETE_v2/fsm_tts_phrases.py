...
Sarvam bulbul:v3 cloud TTS voices (UI labels + API speaker ids, lowercase per REST):
  - shruti → cache dir sarvam_tts_cache/
  - ishita → cache dir sarvam_tts_cache_ishita/ (default: DEFAULT_SARVAM_TTS_SPEAKER_ID)
...
# Sarvam REST `speaker` values (lowercase) and on-screen labels (see index.html / app.js).
SARVAM_AI_OPTUM_VOICES: tuple[tuple[str, str], ...] = (
    ("shruti", "AI Optum: Shruti"),
    ("ishita", "AI Optum: Ishita"),
)
SARVAM_TTS_SPEAKER_IDS: FrozenSet[str] = frozenset(s for s, _ in SARVAM_AI_OPTUM_VOICES)
DEFAULT_SARVAM_TTS_SPEAKER_ID = "ishita"


def sarvam_cache_dir_basename(speaker_id: str) -> str:
    """Filesystem folder under ETE_v2/ for pre-rendered MP3s (one tree per Optum voice)."""
    sid = (speaker_id or "").strip().lower()
    if sid == "ishita":
        return "sarvam_tts_cache_ishita"
    if sid == "shruti":
        return "sarvam_tts_cache"
    raise ValueError(f"Unknown Sarvam AI Optum speaker id: {speaker_id!r}")
...
def collect_all_tts_phrases() -> list[str]:
    """Return sorted unique phrases to pre-generate (Sarvam TTS cache)."""
