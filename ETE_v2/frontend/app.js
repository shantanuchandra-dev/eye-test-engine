...
// Tracks whether current TTS is cached audio (true) or browser speechSynthesis (false).
// When cached, STT should NOT run during TTS (echo cancellation unreliable with HTMLAudio).
// When browser TTS, STT CAN run during TTS (echo cancellation works better).
let _ttsIsCachedAudio = false;

function speakQuestionWithStableFollowup(text, langOverride, onEnd, profileKey = 'default') {
  if (!ttsEnabled) {
    _ttsIsCachedAudio = false;
    if (onEnd) onEnd();
    return;
  }

  _spokenFlowGeneration += 1;
  const sinceGen = _ttsAbortGeneration;

  // Try cached Sarvam/Deepgram TTS first, fall back to browser speechSynthesis
  _ttsIsCachedAudio = true; // assume cached until proven otherwise

  tryPlayCachedTTS(text, () => {
    if (_ttsStale(sinceGen)) return;
    _ttsIsCachedAudio = false;
    if (onEnd) onEnd();
  }, sinceGen).then(played => {
    if (_ttsStale(sinceGen)) return;
    if (!played) {
      // No cached audio — fall back to browser TTS (barge-in safe)
      _ttsIsCachedAudio = false;
...
// isTtsActive() defined at line 118 — checks both speechSynthesis AND _ttsHtmlAudio
...
function bargeInCheckResult(questionToken) {
  ...
  if (isTtsActive()) {
    _ttsSessionId += 1;
    stopHtmlTtsAudio();  // Stop cached MP3 playback (Sarvam/Deepgram)
    try { speechSynthesis.cancel(); } catch (e) {}  // Stop browser TTS
  }
...
async function showQuestion(data) {
  ...
  if (ttsEnabled && !isAutoFlip) {
    // Arm barge-in (only starts STT for browser TTS; skips for cached audio)
    ...
    speakQuestionWithStableFollowup(spokenPrompt, null, () => {
      ...
      // TTS ended. If STT was running (browser TTS), let it continue.
      // If STT was NOT running (cached audio TTS), start it now — zero delay.
      if (recognition) {
        _inputEnabled = true;
        updateVoiceStatus('🎙 Listening...');
        console.log('[Voice] TTS ended — STT continues with pending results');
      } else if (canListen) {
        console.log('[Voice] TTS ended — starting STT fresh');
